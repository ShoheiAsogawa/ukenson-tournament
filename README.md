...
- 観客・選手のスマホから応援コメントを送信し、観客ビュー／OBSオーバーレイに弾幕として流す（危険語は `*` に置換・レート制限・運営の停止スイッチ付き）
...
> Realtime publication には `tournament_states` と `cheer_comments` の両方が入ります（既存プロジェクトは `supabase/production-hardening.sql` を実行）。
...
1. Run `supabase/schema.sql` in Supabase SQL Editor (or at least `supabase/production-hardening.sql` on existing projects).
...
current match assigned to that table. Treat table URLs as semi-secret and avoid
...
Free projects pause after 1 week of inactivity — wake the project before the event.

## Broadcast overlay (OBS)

`#/overlay` is a **cheer-comment-only** transparent overlay for OBS Browser Sources.
...
- Only danmaku comments flow across the screen
- Respects the operator cheer on/off switch
- Open from 運営モード > 配信・画面出力
...
`public.cheer_comments`, and every open spectator / broadcast overlay receives them live via
Supabase Realtime.

Setup:

1. Re-run `supabase/cheer-comments-setup.sql` (or `schema.sql` / `production-hardening.sql`)
   so both `cheer_comments` and `tournament_states` are in the `supabase_realtime` publication.
...
mode) and still applies local dangerous-word masking.
